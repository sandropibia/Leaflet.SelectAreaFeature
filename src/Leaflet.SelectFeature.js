(function (factory) {
    ...
    "use strict";
    L.SelectFeature = L.Handler.extend({
    ...
    initialize: function (map, options) {
		...
		this._bound_layers = [];
		this._contains_layers = [];
		this._area_pologon_layers = [];
		
		this._area_line = '';
		this._area_line_new = '';
		
		this._area_layergroup = '';
    ...
    doRemoveAllSelection: function() {
        ...
        for (index = 0; index < this._area_pologon_layers.length; ++index) {
		  this._map.removeLayer(this._area_pologon_layers[index]);
		  delete this._area_pologon_layers[index];
        }
	},
...
L.Map.addInitHook('addHandler', 'selectFeature', L.SelectFeature);
